...
def writeToFile(name, total_payable, monthly_payments):

    # Open the output file in append mode, so that file isn't overwritten everytime we output data
    with open('output.csv', mode='a', newline='') as file:
        data_writer = csv.writer(file)
        data_writer.writerow([name, total_payable, monthly_payments])  # Write persons name and total payable amount to the file     
